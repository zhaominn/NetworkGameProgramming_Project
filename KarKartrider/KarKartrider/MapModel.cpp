...
MapModel::MapModel(string name, string path, string obj_name, string obj_type, bool rigid_status, glm::mat4 start_matrix)
{
...
void MapModel::load_obj(string name, string path, string obj_name, string obj_type, glm::mat4 start_matrix)
{
	read_obj_file(name, path, this, obj_name, obj_type);
	this->matrix = start_matrix * this->matrix;
}

const void MapModel::draw(GLint shaderProgramID, bool(*isKeyPressed_s)(const char&))
{
...
const void MapModel::draw_rigidBody(GLuint shaderProgramID)
{
	if (this->rigidBody) {
